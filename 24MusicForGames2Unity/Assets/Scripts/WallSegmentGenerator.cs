...
public class WallSegmentGenerator : MonoBehaviour
{
    ...
    public float overlapAngle = 0.5f;
    ...
    GameObject CreateDoorSegmentPrefab()
    {
        ...
        segment.AddComponent<MeshRenderer>();
        segment.AddComponent<BoxCollider>(); // Add a box collider
        segment.AddComponent<SimpleDoorTrigger>(); // Add door trigger script
    ...
    float xCenter = Mathf.Cos(midAngle) * (outerRadius + innerRadius) / 2;
    float zCenter = Mathf.Sin(midAngle) * (outerRadius + innerRadius) / 2;
    boxCollider.center = new Vector3(xCenter, doorHeight / 2, zCenter);
    boxCollider.size = new Vector3(doorWidth, doorHeight, Mathf.Abs(Mathf.Cos(angle) - Mathf.Cos(nextAngle)) * outerRadius);
    #if UNITY_EDITOR
...
void CreateDoorSegment(List<Vector3> vertices, List<int> triangles, List<Vector2> uvs, float angle, float nextAngle, float doorWidth, float doorHeight, float height)
{
    ...
    float innerZ2 = Mathf.Sin(doorCenterAngle + halfWidth / innerRadius) * innerRadius;
    float outerX1 = Mathf.Cos(doorCenterAngle - halfWidth / outerRadius) * outerRadius;
...
WallSegmentGenerator script = (WallSegmentGenerator)target;
